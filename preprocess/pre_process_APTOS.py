...
DATA_PATH_META = '/l/users/xiangjian.hou/hc701-fed_data/aptos/'
SAVE_PATH_META = '/l/users/xiangjian.hou/preprocessed/aptos/'

class pre_process_messidor(object):
    def __init__(self, data_path):
        self.data_path = data_path
        self.train_path = os.path.join(self.data_path, 'train_images')
        self.test_path = os.path.join(self.data_path, 'test_images')
        self.train_info = pd.read_csv(os.path.join(self.data_path, 'train.csv'))

    def process_train(self, pre_processed_path):
        save_path = os.path.join(pre_processed_path, 'train')
        for i,j in zip(self.train_info['id_code'],self.train_info['diagnosis']):
            pre_processed_data = {}
            origin_data = Image.open(os.path.join(self.train_path, i+'.png'))
