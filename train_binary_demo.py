    ...
    for noise_scale in [0.5,1,3,5,10,20]:
        for seed in [42,43,44]:
            ...
            def apply_dp(update_tensor,total_round,clip_threshold=clip_value,learn_rate=lr,dp_std=std,dp_mean=mean):
                ...
                # add noise
                update_tensor_clip+=torch.normal(mean=dp_mean, std=learn_rate*dp_std*clip_threshold*torch.sqrt(torch.tensor(total_round)),size=update_tensor_clip.shape).to('cuda:1')
